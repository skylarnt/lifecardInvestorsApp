                                            ...
                                            <tr v-for="(p, index) in property.data" :key="p.id">
                                                ...
                                                <td>
                                                    <span class="d-flex pt-2">
                                                        <span>
                                                            <span class="b-avatar rounded-circle"
                                                        ...
                                                        <span class="ml-3">
                                                            <!-- <p class="mb-0"> -->
                                                            <span class="fw-semi-bold">&nbsp; {{ p.marketer.fullname
                                                            }}</span>
                                                            <!-- </p> -->
                                                            <p>
                                                                <small>
                                                ...
                                                <td>
                                                    <span class="badge badge-pill" :class="p.status == 'pending'
                                                        ? 'badge-light-warning'
                                                        : 'badge-light-info'
                                                        ">
                                                        {{ p.status }}
...
export default {
    ...
    methods: {
        ...
        fetchData(page = 1) {
            this.loading = true
            this.$api
                .post(this.dynamic_route(`/properties/manage-marketers-property?page=${page}`), { filter: this.filter })
                .then(res => {
                    if (res.status == 200) {
        ...
        toggle_status(status) {
            this.loading = true
            this.$api.post(this.dynamic_route(`/properties/toggle-status/${this.current.property_id}`), { status: status })
                .then((res) => {
        ...
        toggle_status(status) {
            this.loading = true
            this.$api.post(this.dynamic_route(`/properties/toggle-status/${this.current.property_id}/${this.marketer.marketer_id}`), { status: status })
                .then((res) => {
                    this.$toast.success(res.data.message, {
                        position: 'top-right',
                        timeout: 5000,
