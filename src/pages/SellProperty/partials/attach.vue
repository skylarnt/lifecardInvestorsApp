                    ...
                    <table class="table table-hover table-sm mb-0 requests-table">
                        ...
                        <tbody>
                            <tr v-for="(user, index) in addmarketers" :key="user.id">
                                ...
                                <td>
                                    <button class="fa fa-trash fa-2x text-center btn"
                                        @click="settCurrent(user); openConfirm = true;"></button>
                                </td>
                                <!-- <td class="text-center" style="font-size: 18px">
                                    <v-btn outlined color="blue" @click="addMarketer(user)">
                                        Add
            ...
            <v-dialog v-model="openConfirm" max-width="390">
                <v-card>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="primary darken-1" text @click="deleteMarketer(user); openConfirm = false">
                            Yes, continue
                        </v-btn>
                    </v-card-actions>
                </v-card>
...
export default {
    ...
    methods: {
        ...
        getPropertyMarketers() {
            this.$api
                .post(this.dynamic_auth_route(`/getmarketer-property/${this.property.investor_property_id}`))
                .then((res) => {
        ...
        attachMarkerter() {
            this.loading = true
            this.$api
                .post(this.dynamic_route(`/properties/attach-marketers/${this.property.investor_property_id}`), this.addmarketers)
                .then((res) => {
                    this.loading = false;
                    this.closeMe();
        ...
        deleteMarketer(data) {
            ...
            this.$api
                .post(this.dynamic_route(`/properties/delete-marketers/${this.property.investor_property_id}`), payload)
                .then((res) => {
                    this.loading = false;
                    this.closeMe();
                    this.$toast.success(res.data.message, {
