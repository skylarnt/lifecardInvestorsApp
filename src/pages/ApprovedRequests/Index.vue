<template>
  <div>
    <v-app v-if="auth_data && auth_data.user_type != 'admin'" class="p-4">
        ...
        <b-col>
          <Widget title="" customHeader>
            <h5 class="d-inline-block">
              Approved <span class="fw-semi-bold">Property Requests</span>
            </h5>
      ...
      <b-modal
        size="lg"
        :title="`Payment breakdown for ${
          current.request && current.request.name
        }  request`"
        id="breakdown"
        hide-footer
      >
...
export default {
  ...
  methods: {
    ...
    fetchData(page = 1) {
      this.loading = true;
      this.$api
        .post(this.dynamic_route(`/requests/approved_request?page${page}`), {
