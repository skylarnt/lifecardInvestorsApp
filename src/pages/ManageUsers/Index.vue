<template>
  <div class="tables-basic" data-app>
                      ...
                      <v-list>
                        ...
                        <v-list-item>
                          <v-list-item-title
                            ...
                            "
                          >
                            <i class="mdi mdi-account-outline mr-1"></i> Reset
                            Password
                          </v-list-item-title>
                        </v-list-item>
                      </v-list>
    ...
    <b-modal
      size="xl"
      style="background: white"
      :title="`View ${currentUser.fullname} Investments`"
      id="investments"
      hide-footer
    >
    ...
    <v-dialog v-model="openConfirm" max-width="550">
      <v-card>
        <v-card-title class="text-h5 text-success">
          {{ dynamic_status == "inactive" ? "Deactivate" : "Activate" }} &nbsp;
    ...
    <!-- delete user dialog -->
    <v-dialog v-model="openConfirm2" max-width="485">
      <v-card>
        <v-card-title class="text-h5 text-success">
          <p>Delete User account.</p>
          <p>
            <small
              ><b>Note: </b> This action is irreversible and will delete all
              records associated with this user including transactions, groups,
              profile etc.</small
            >
          </p>
        </v-card-title>
    ...
    <v-dialog v-model="openConfirm3" max-width="450">
      <v-card>
        <v-card-title class="text-h5">
          <p class="text-success">
            Reset <b>{{ currentUser.fullname }}</b
            >'s password
          </p>
          <p>
            <small
              ><b>Note: </b> This will change user password to
              <b>12345678</b></small
            >
          </p>
        </v-card-title>
    ...
    <v-dialog v-model="openConfirm4" max-width="450">
      <v-card>
        <v-card-title class="text-h5">
          <p class="text-success">
            You are about to <b> Login As {{ currentUser.fullname }}</b>
          </p>
          <p>
            <small
              ><b>Note: </b> This will make you act as
              <b>{{ currentUser.fullname }}</b></small
            >
          </p>
        </v-card-title>
...
export default {
  ...
  methods: {
    ...
    getUsers(page = 1) {
      ...
      this.$api
        .post(this.dynamic_route(`/users/fetch?page=${page}`), {
          filter: this.filter,
        })
        .then((res) => {
          this.loading = false;
    ...
    toggle_status(id, status) {
      ...
      axios
        .put(
          this.dynamic_route(`/users/toggle-status/${id}`),
          { status },
          {
            headers: {
    ...
    deleteUser(data) {
      swal({
        title: "Are you sure?",
        text: "Once deleted, you will not be able to recover this user and he/she requests!",
        icon: "warning",
        buttons: true,
        dangerMode: true,
      }).then((willDelete) => {
