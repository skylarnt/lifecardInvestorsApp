<template>
  <v-app class="p-4">
      ...
      <b-col>
        <Widget title="" customHeader>
          <h5 class="d-inline-block">
            Manage <span class="fw-semi-bold">Approved Property Requests</span>
          </h5>
          ...
          <div class="mt-3" v-if="approvedRequests.data.length">
            <div class="row">
              <div
                class="col-md-6 mb-2"
                v-for="(p, i) in approvedRequests.data"
                :key="i"
              >
                <div class="card">
                  <div class="card-body">
                        ...
                        <td>
                          ...
                          <p>
                            <span
                              ...
                              :class="{
                                'badge-warning':
                                  p.contract_recieved == 'pending',
                                'badge-secondary':
                                  p.contract_recieved == 'sent',
                                'badge-success':
                                  p.contract_recieved == 'submitted',
                              }"
                            >
                              {{ p.contract_recieved }}
                            </span>
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <p>
                            ...
                            >
                              {{
                                p.allocation_type != null
                                  ? p.allocation_type == "online"
                                    ? "Paper"
                                    : "Physical"
                                  : "Unknown"
                              }}
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          <p>
                            ...
                            >
                              {{
                                p.deed_of_assignment == "not_assigned"
                                  ? "Yet to assign"
                                  : "Assigned"
                              }}
                            </span>
                  ...
                  >
                    <span class="s" v-if="p.investor_property_id == null">
                      Special request
                    </span>
    ...
    <b-modal
      :title="`Record Transaction for request ${current.name} `"
      id="record"
      hide-footer
    >
      <record
    ...
    <b-modal
      :title="`Allocate property  ${current.name} to user ${
        current.request && current.request.investor.fname
      } ${current.request && current.request.investor.lname}`"
      id="allocate"
      hide-footer
    >
      <allocate
    ...
    <b-modal
      :title="`Deed of assignment  type for request  ${current.name} to user ${
        current.request && current.request.investor.fname
      } ${current.request && current.request.investor.lname}`"
      id="assignment_type"
      hide-footer
    ...
    <b-modal
      :title="`Survey plan for  ${current.name} to user ${
        current.request && current.request.investor.fname
      } ${current.request && current.request.investor.lname}`"
      id="survey"
      hide-footer
    ...
    <b-modal
      size="lg"
      :title="`View Transaction breakdown for ${
        current.request && current.request.investor.fname
      } `"
      id="breakdownA"
      hide-footer
    ...
    <v-dialog v-model="openConfirm3" max-width="390">
      <v-card>
        <v-card-title class="text-h5"> Assign house key to user </v-card-title>
        ...
        <v-card-actions>
          ...
          <v-btn color=" darken-1" text @click="openConfirm3 = false">
            No, exit
          </v-btn>
    ...
    <v-dialog v-model="openConfirm4" max-width="390">
      <v-card>
        <v-card-title class="text-h5"> Assign deed of assignment </v-card-title>
...
export default {
  ...
  methods: {
    ...
    toggle_status(id, status) {
      this.loading = true;
      this.$api
        .post(this.dynamic_route("/requests/admin/change_contract_status"), {
          id,
          status,
        })
        .then((res) => {
    ...
    assignHouseKey(id) {
      this.loading = true;
      this.$api
        .post(this.dynamic_route("/properties/admin/assign_key"), {
          id,
        })
        .then((res) => {
          this.loading = false;
    ...
    deedOfAssignment(id) {
      if (this.current.type == "land") {
        ...
        this.$api
          .post(
            this.dynamic_route("/properties/admin/assign_deed_of_assignment"),
            {
              id,
            }
          )
