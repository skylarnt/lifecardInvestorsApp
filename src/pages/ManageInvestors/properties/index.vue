<template>
  <v-app class="p-4">
      ...
      <b-col>
        <Widget title="" customHeader>
          <h5 class="d-inline-block">
            Manage Investors <span class="fw-semi-bold">Properties</span>
          </h5>
          <v-btn
            depressed
            color="primary"
                        ...
                        <v-list>
                          <v-list-item>
                            <v-list-item-title
                              ...
                              "
                            >
                              <i class="mdi mdi-eye-outline mr-1"></i> View more
                              info
                            </v-list-item-title>
                          </v-list-item>
                          <v-list-item>
                            <v-list-item-title
                              style="cursor: pointer"
                              @click="
                                pass_current_img(p, p.name, 'change');
                                $bvModal.show('change_image');
                              "
                            >
                          ...
                          <v-list-item v-if="p.status == 'active'">
                            <v-list-item-title
                              ...
                              @click="
                                openConfirm = true;
                                dynamic_status = 'inactive';
                                status_id = p.id;
                              "
                            >
                              <button class="text-left p-0 btn">
                          ...
                          <v-list-item>
                            ...
                            >
                              <i
                                class="mdi mdi-delete-forever-outline mr-1"
                              ></i>
                              Delete
                            </v-list-item-title>
                          </v-list-item>
    ...
    <b-modal
      size="lg"
      style="background: white"
      title="Chnage or remove Main Property Image"
      id="change_image"
      hide-footer
    >
      <ChangeImage
...
export default {
  ...
  methods: {
    ...
    fetchData(page = 1) {
      ...
      axios
        .post(
          this.dynamic_route("/properties/admin/all"),
          this.filters,
          {
            headers: {
    ...
    updateLink() {
      ...
      this.$api
        .post(
          this.dynamic_route(
            "/properties/admin/property_link/" + this.current.id
          ),
          { property_link: this.current.property_link }
        )
        .then((res) => {
          this.loading1 = false;
    ...
    toggle_status(id, status) {
      ...
      axios
        .put(
          this.dynamic_route(`/properties/admin/toggle-status/${id}`),
          { id, status },
          {
            headers: {
              authorization: `Bearer ${this.auth_token}`,
        ...
        .catch((err) => {
          ...
          if (err.response.status == 500) {
            return this.$toast.error(
              "Unable to delete property  it has been used in many places!",
              {
                position: "top-center",
                timeout: 5000,
                closeOnClick: true,
